#!/usr/bin/env bash
#
# go-shlex/mayhem/test.sh — RUN google/shlex's OWN Go test suite (`go test ./...`) and emit a
# CTRF summary. exit 0 iff no test failed.
#
# PATCH-grade oracle: shlex's suite is a REAL known-answer suite — TestClassifier asserts the
# rune-class mapping, and TestTokenizer / TestLexer / TestSplit assert the exact token sequence
# produced for a fixed shell-style input string (quotes, escapes, comments, '=' and '/' words).
# They assert BEHAVIOUR, not "exits 0", so a no-op / stubbed lexer that breaks tokenization FAILS
# this oracle. This script only RUNS the suite (the project's own normal-flags suite — no
...
# ── Behavioral probe via the dynamically-linked fuzz_lexer binary (anti-reward-hacking, §6.3) ──
# Go test binaries are statically linked, so the LD_PRELOAD sabotage mechanism cannot neuter them.
# /mayhem/fuzz_lexer IS dynamically linked (built with clang+ASan). Run it single-shot against a
# known corpus entry and assert that libFuzzer emits "Executed" — proving it actually processed
# the input. The sabotage LD_PRELOAD neuters fuzz_lexer (not in /usr/bin etc.), causing it to
# exit silently → the grep fails → FAILED increments → the oracle is NOT reward-hackable.
PROBE_INPUT="$SRC/mayhem/fuzz_lexer/testsuite/test_string"
if [ -x /mayhem/fuzz_lexer ] && [ -f "$PROBE_INPUT" ]; then
  echo "=== behavioral probe: fuzz_lexer single-shot on known corpus ==="
  PROBE_OUT=$(/mayhem/fuzz_lexer "$PROBE_INPUT" 2>&1 || true)
  if echo "$PROBE_OUT" | grep -q "Executed"; then
    echo "PROBE PASS: fuzz_lexer executed the corpus input (lexer active)"
    PASSED=$(( PASSED + 1 ))
  else
    echo "PROBE FAIL: fuzz_lexer produced no 'Executed' output (lexer inactive or sabotaged)"
